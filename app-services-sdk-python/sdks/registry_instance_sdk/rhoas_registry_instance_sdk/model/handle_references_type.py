...
class HandleReferencesType(ModelSimple):
    ...
    def __init__(self, *args, **kwargs):
        """HandleReferencesType - a model defined in OpenAPI

        Note that value can be passed either in args or in kwargs, but not in both.

        Args:
            args[0] (str): How to handle references when retrieving content.  References can either be left unchanged (`PRESERVE`), re-written so they are valid in the context of the registry (`REWRITE`), or fully dereferenced such that all externally referenced content is internalized (`DEREFERENCE`).., must be one of ["PRESERVE", "DEREFERENCE", "REWRITE", ]  # noqa: E501
